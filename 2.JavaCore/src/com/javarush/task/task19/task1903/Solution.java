...
public class Solution {
    ...
    public static class IncomeDataAdapter implements Customer, Contact {
        ...
        public String getPhoneNumber() {
            ...
            /*int phCode = data.getCountryPhoneCode();
            String nools = "";
            String phNumber = Integer.toString(data.getPhoneNumber());
                for (int i = 0; i < (9 - phNumber.length()); i++)
                {
                    nools = nools + "0";
                }
            phNumber = phNumber + nools;

            return "+" + phCode
                    + "(0"
                    + phNumber.substring(0, 2)
                    + ")"
                    + phNumber.substring(2, 5)
                    + "-"
                    + phNumber.substring(5, 7)
                    + "-"
                    + phNumber.substring(7);*/
        }
    }
